...
A tag is metadata attached to a note via `#hashtag` syntax in the note body. A note can have zero or many tags. Tags are user-created — adding `#Medical` to a note's text creates the "Medical" tag.
...
- **Tags on a note** — there is no `tags` property. Tags exist only as `#hashtag` text in the note body and as Smart Folders.
